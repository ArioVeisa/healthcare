      ...
      <div className="bg-medical-panel w-full max-w-2xl border border-medical-accent-dark/30 rounded-2xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className={`p-6 border-b flex justify-between items-center ${currentStyle}`}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold tracking-wide">Data Pasien: {patient.name}</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-4">
            <InfoCard label="Media Terlampir">
              <div className="flex items-center gap-3">
                <span className={`flex items-center gap-1 text-sm font-medium ${patient.has_audio ? 'text-blue-400' : 'text-slate-600'}`}>
                  <Mic className="w-4 h-4" />
                  {patient.has_audio ? 'Audio' : 'Tidak ada audio'}
                </span>
                <span className={`flex items-center gap-1 text-sm font-medium ${patient.has_image ? 'text-purple-400' : 'text-slate-600'}`}>
                  <Image className="w-4 h-4" />
                  {patient.has_image ? 'Gambar' : 'Tidak ada gambar'}
                </span>
              </div>
            </InfoCard>
            <InfoCard label="Status Booking">
              <div className="flex items-center gap-2">
                ...
                <span className={`font-semibold ${bk.cls}`}>{bk.text}</span>
              </div>
            </InfoCard>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <InfoCard label="Waktu Masuk">
              <div className="flex items-center gap-2 text-sm">
                ...
                {formatDT(patient.created_at)}
              </div>
            </InfoCard>
            <InfoCard label="Terakhir Update">
              <div className="flex items-center gap-2 text-sm">
                <Clock className="w-4 h-4 text-slate-500" />
                {formatDT(patient.updated_at)}
              </div>
            </InfoCard>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-medical-dark p-4 rounded-xl border border-medical-accent-dark/50 text-medical-accent-light min-h-[60px]">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-medical-dark p-4 rounded-xl border border-medical-accent-dark/50 text-medical-accent-light leading-relaxed font-light border-l-2 border-l-emerald-500 min-h-[60px]">
...
function InfoCard({ label, children }) {
  return (
    <div className="bg-medical-dark p-4 rounded-xl border border-medical-accent-dark/50">
      <span className="text-medical-accent text-xs font-semibold uppercase tracking-wider block mb-2">
        {label}
      </span>
      {children}
    </div>
